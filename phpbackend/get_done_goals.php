...
if (isset($_POST['project_id'])) {
    ...
    $result = mysql_query("SELECT g.goal_id, g.goal_name, g.user_id, tux.full_name, g.date_created, g.date_finished FROM `goal` g LEFT JOIN (SELECT tu.user_id, tu.full_name from tbl_usersinfo tu) tux ON tux.user_id = g.user_id WHERE project_id = '$project_id' and status = 'Done'");
    

    if(mysql_num_rows($result)>0){

        $response["done_goals"] = array();
        ...
        while($row = mysql_fetch_array($result)){
            ...
            $goal["date_created"] = 'Completed last '.$phpdatex;  
			...
			array_push($response["done_goals"], $goal);
