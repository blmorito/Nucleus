...
if (isset($_POST['user_id'])) {
    ...
    $result = mysql_query("SELECT * FROM tbl_usersinfo LEFT JOIN tbl_users ON tbl_users.user_id = tbl_usersinfo.user_id LEFT JOIN avatars ON tbl_usersinfo.avatar_id = avatars.avatar_id where tbl_usersinfo.user_id = '$user_id'") or trigger_error(mysql_error());

    if(mysql_num_rows($result)>0){

        $row = mysql_fetch_array($result);
        $response["full_name"] = $row["full_name"];
        $response["address"] = $row["comp_org"];
